...
The idea is that idea is that given some values of a function, we can find a polynomial which passes through them. The example given in \citep{lissamen2004mei} is to consider the growth rate per year for a group of young people. The results are given in \cref{table:heights} and can be plotted as five points on a graph. The question asked, and answered, in \citep{lissamen2004mei} is can a formula be found which fits all of these points. The formula given in \citep{lissamen2004mei} is
\begin{equation*}
...
\begin{table}[ht]
\centering
\caption{Measurements of the average height increase per year at various ages taken from \citep{lissamen2004mei}.}
...
Following \citep{lissamen2004mei}, we will use the notation in \cref{table:notation}. We start our labelling from the left most point and moving rightwards until we get to the last point. Here we are taking $x$ values that have a constant spacing which we denote by $h$\footnote{If this seems suggestive of our notation when doing differentiation from first principles, that is on purpose. The separation between points is precisely what we want to take to zero to get the derivative. }.\\
...
This works as follows. We have $n=4$ sub intervals defined by the five values of $x$ we are given. Thus we can use \cref{eq: trapezium rule}, and $\Delta x=(3-1)/4=1/2$ to find
\begin{align*}
A_{T} &=\frac{\Delta x}{2}\left(f_{0}+2\left(f_{1}+f_{2}+f_{3}\right)+f_{4}\right)\\
	&=\frac{1}{4}\left(0+2\left(0.41+0.69+0.92\right)+1.10\right)\\
	&=1.29.
\end{align*}
...
We now turn to numerical approaches to differentiation. We have seen hints of this in the background when discussing other topics, including as far back as when we discussed differentiation from first principles.\\

A major reason that we need a numerical approach to differentiation is that the definition of differentiation from first principles is very hard to implement on a computer.  It is much easier for a computer to understand an approximate method. \\


\subsection*{Forward difference method}
The key idea is to look at the definition of differentiation from first principles in \cref{eq: derivative definition}, and drop the limit part. e.g. we calculate an approximate gradient for points a small difference apart. In other words,
\begin{equation}
\frac{\ud f}{\ud x}\simeq \frac{f(x+h)-f(x)}{h}=\frac{\Delta f}{h},
\label{eq: forward difference approx}
\end{equation}
for $h$ a small step between neighbouring points. This is known as the \textbf{forward difference method}, since we are looking at the difference between the function at the point $x$ and at the point $x+h$ one step ahead of $x$. This is shown schematically in \cref{fig: forward difference approximation}.\\


\begin{figure}[ht]
    \centering
\ThisAltText{ schematic of the forward difference approximation.}
\begin{tikzpicture}
\begin{axis}[%title={Forward difference method},
ticks=none,
clip=false,
xlabel={$x$},
xmin=0,
...
axis x line=bottom,
axis y line = left,
line width=1pt,line cap=round,line join=round, smooth]
...
\fill [name intersections={of=StraightLine and CurveLine, name=i, total=\t}][CDred] 
    \foreach \s in {1,...,\t}{(i-\s) circle (1.8pt)
        node (\s) {}};

%%%% Dashed lines and labels
\begin{scope}[dashed,very thin]

    \node[below=1.25cm of 1] (a) {$x$};
    \node (b) at (a -| 2) {$x+h$};

    \draw (1) -- (a);
    \draw (2) -- (b);

    \node[left=1.2cm of 1] (c) {$f(x)$};
    \node (d) at (c |- 2) {$f(x+h)$};
...
\draw[<->] (a) --node[midway,fill=white]{$h$} (b);
\draw[<->] (c) --node[midway,fill=white]{$\Delta f$} (d);

%\node at (4.5,4.5) {$\dfrac{\mathrm{d}u}{\mathrm{d}t}\approx\dfrac{\Delta u}{\Delta t}=\dfrac{u_{n+1}-u_n}{t_{n+1}-t_n}$};

\end{axis}
\end{tikzpicture}
\caption{A schematic of the forward difference method showing how to approximate the tangent line to the curve at $x$ }
        \label{fig: forward difference approximation}
\end{figure}
...
\begin{ex}
Consider the function $f(x)=x^{2}\cos(x)$, we can calculate the approximate derivative at $x=2$ using \cref{eq: forward difference approx} and taking the step size to be $h=0.1, 0.05, 0.025, 0.0125$. We do this as follows:
\begin{itemize}
\item $h=0.1$
\begin{equation*}
...
\begin{align*}
\frac{\ud f}{\ud x}(x)&=2x\cos(x)-x^{2}\sin(x),\\
...
\begin{exercise}
Use the forward difference method with $h=0.05$ to approximate the derivative of $f(x)=4e^{2x}$ at $x=1$.
\end{exercise}


The forward difference method is not the only approach to approximating the gradient of the tangent line at a point. There are two other popular approaches, the \textbf{central difference method} and the \textbf{backwards difference method}. 

\subsection*{Backwards difference method}

The backwards difference method is the \textit{reverse} of the forward difference method. The picture to have in mind, shown in \cref{fig: backwards difference approximation}, is essentially the same as in \cref{fig: forward difference approximation}. The difference is that now we are comparing the value of the function at point with its value at the point one step earlier. As an equation this is 

\begin{equation}
\frac{\ud f}{\ud x}\simeq \frac{f(x)-f(x-h)}{h}.
\label{eq: backwardsdifference approx}
\end{equation}

\begin{figure}[ht]
    \centering
\ThisAltText{Schematic of the backwards difference approximation.}
\begin{tikzpicture}
\begin{axis}[%title={Forward difference method},
ticks=none,
clip=false,
...
\begin{scope}[dashed,very thin]
    ...
    \node[below=1.25cm of 1] (a) {$x-h$};
    \node (b) at (a -| 2) {$x$};

    \draw (1) -- (a);
    \draw (2) -- (b);

    \node[left=1.2cm of 1] (c) {$f(x-h)$};
    \node (d) at (c |- 2) {$f(x)$};
...
\end{axis}
\end{tikzpicture}
\caption{A schematic of the backwards difference method showing how to approximate the tangent line to the curve at $x$ }
        \label{fig: backwards difference approximation}
\end{figure}

\begin{ex}
Similar to in the previous case, consider the function $f(x)=x^{2}\cos(x)$, we can calculate the approximate derivative at $x=2$ using the backwards difference method and taking the step size to be $h=0.1, 0.05, 0.025, 0.0125$. We do this as follows:
\begin{itemize}
\item $h=0.1$
\begin{equation*}
...
The central difference method is essentially a combination of the other two methods. In this case we approximate the gradient of the tangent line by using the straight line between the two points $f(x-h)$ and $f(x+h)$. In this case we are approximating the derivative by the gradient,
\begin{equation}
\frac{\ud f}{\ud x}\simeq \frac{\Delta f}{\Delta x}=\frac{f(x+h)-f(x-h)}{(x+h)-(x+h)}=\frac{f(x+h)-f(x-h)}{2h}.
\label{eq: central difference method}
\end{equation}

The central difference method is often preferred to the forward and backwards difference method, since it is a \textit{higher order method}. This means that when we discuss the error we will see that it is quadratic in the small parameter $h$ while the other methods are linear in $h$. 


\begin{ex}
Consider $f(x)=\tan(x)$ and use the central difference method with $h=0.1, 0.05, 0.025, 0.0125$ to approximate the derivative at $x=1$. This gives:
\begin{itemize}
\item $h=0.1$
\begin{equation*}
...
\begin{ex}
Use both the forward and central difference methods to approximate the derivative of $f(x)=\cos(x)$ at $x=\uppi/4$ using the step sizes $h=0.1$ and $h=0.05$. Compare these to each other and to the exact answer.
\end{ex}

\begin{ex}
Pick your favourite programming language and write a function which implements the central difference method. Check it against the example that we computed above, then check a the cases that we explored using the forward and backwards difference method.
\end{ex}
...
\end{equation}
The $\vert \vert$ mean that this is the absolute value, and of the number within and is always positive. In other words we care about how far away our approximate value is from the true value, but not whether it is greater than or less than the true answer.\\
...
There is a discussion of error and the related concept of convergence of an approximation, i.e. whether the approximation is closing in on the true value and how quickly, in  \citep{lissamen2004mei}.

\begin{ex}
Consider the examples of approximating the derivative of $x^{2}\cos(x)$ using both the backwards and forwards difference method with $h=0.0125$.  The relative and absolute errors are shown in \cref{table: percentage error 1}.

\begin{table}[ht]
\centering
\caption{A comparison of our approximations to the derivative of $x^{2}\cos(x)$ at $x=2$ and the percentage error in each case.}

\vspace{2mm}

\label{table: percentage error 1}



\begin{tabular}{|c|c|c|c|} 
 \hline
& Approximate derivative &Relative absolute error & Percentage error\\
 \hline
Forward difference& $-5.34196$ &$0.00758 $ & $0.758\% $\\
 \hline
Backward difference & $-5.26143$ & $0.00761$ & $0.761\%$\\
 \hline
\end{tabular}
\end{table}
...
\begin{exercise}
Go back to all of the examples of using the forward, backwards, and central difference equation and calculate the relative and percentage error in each case. 
\end{exercise}
